        ...
        for name in spec.actuators.keys():
            if mujoco.mj_name2id(mjcf_model, mujoco.mjtObj.mjOBJ_ACTUATOR, name) >= 0:
                ...
                if mujoco.mj_name2id(mjcf_model, mujoco.mjtObj.mjOBJ_ACTUATOR, prefixed) >= 0:
                    ...
                    # Search matching actuator
                    found = False
                    for a_id in range(mjcf_model.nu):
                        a_name = mujoco.mj_id2name(mjcf_model, mujoco.mjtObj.mjOBJ_ACTUATOR, a_id) or ""
                        if name in a_name or name.replace("rh_", "") in a_name:
                            actuator_names_list.append(a_name)
                            found = True
                            break
                    if not found:
                        raise ConfigError(f"Cannot resolve spec actuator '{name}' in compiled MuJoCo model")
