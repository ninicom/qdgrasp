...
def solve_region_dls_ik_batch(
    ...
    def compute_current(q_batch):
        ...
        for tip in spec.fingertip_links:
            ...
            norm_list.append(T[:, :3, 2] * normal_weight)
    ...
    for it in range(max_iter):
        ...
        with torch.no_grad():
            ...
            required_tips = num_tips if num_tips <= 3 else (num_tips - 1)
            p_converged = torch.sum(p_errs < pos_tolerance, dim=1) >= required_tips
            n_converged = torch.sum(n_dots > normal_tolerance_dot, dim=1) >= required_tips
            ...
            target_flat = torch.cat([p_target.view(B, -1), (t_target_normal * normal_weight).view(B, -1)], dim=1)
