...
def select_target_uniform_visible(obs: SceneObservation, rng: np.random.Generator, min_visibility: float = 0.05) -> str:
    ...
    """
    candidates = [
        obj_id for obj_id, vis in obs.visibility_by_object.items()
...
def select_target_difficulty_weighted(obs: SceneObservation, rng: np.random.Generator, min_visibility: float = 0.05) -> str:
    """
...
def select_target_declutter_ordered(obs: SceneObservation, min_visibility: float = 0.05) -> str:
    ...
    """
    candidates = [
        (vis, obj_id) for obj_id, vis in obs.visibility_by_object.items()
        if vis >= min_visibility
    ]
    if not candidates:
        raise ValueError("No objects meet the minimum visibility threshold.")
