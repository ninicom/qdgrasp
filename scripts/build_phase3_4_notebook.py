...
"""
        ...
        _code(
            'import shutil\nimport subprocess\nimport sys\n\n# Section 3.3 step 6: does the defect reproduce with no QDGrasp backend in the\n# call path? Everything runs in a subprocess with cwd at the repo, the same way\n# the other cells do -- importing qdgrasp into the notebook process after a\n# sys.path insert collides with the installed copy.\nprint("=" * 70)\nprint("upstream-only reproduction")\nprint("mjwarp-testspeed:", shutil.which("mjwarp-testspeed") or "not on PATH")\n\nprobe = (\n    "import mujoco, mujoco_warp, sys\\n"\n    "from qdgrasp.dataset.pipeline.generated_reachable import "\n    "build_generated_reachable_object as f\\n"\n    "from qdgrasp.dataset.pipeline.validators.mujoco_rollout import "\n    "build_rollout_scene_model as b\\n"\n    "from qdgrasp.robot.spec import RobotSpec, resolve_robot_asset\\n"\n    "s = RobotSpec.from_config(\'leap_hand.yaml\', sample_anchors=False)\\n"\n    "x = f(\'leap_hand\')\\n"\n    "m = b(resolve_robot_asset(s.config.source_asset), x.collision_geoms,"\n    " object_pos=x.object_pos, object_mass=x.mass)\\n"\n    "print(\'model ngeom\', m.ngeom, \'nq\', m.nq, \'nu\', m.nu)\\n"\n    "d = mujoco.MjData(m); mujoco.mj_forward(m, d)\\n"\n    "wm = mujoco_warp.put_model(m)\\n"\n    "wd = mujoco_warp.put_data(m, d, nworld=4)\\n"\n    "for _ in range(8):\\n"\n    "    mujoco_warp.step(wm, wd)\\n"\n    "print(\'upstream path stepped 8 times over 4 worlds, no QDGrasp backend\')\\n"\n)\n\nsanitizer = shutil.which("compute-sanitizer")\nif not sanitizer:\n    print("compute-sanitizer unavailable; cannot separate upstream from wrapper here")\nelse:\n    run = subprocess.run(\n        [sanitizer, "--tool", "initcheck", "--error-exitcode", "0",\n         "--print-limit", "6", sys.executable, "-c", probe],\n        cwd="/tmp/qdgrasp_repo", capture_output=True, text=True, timeout=5400,\n    )\n    out = run.stdout + run.stderr\n    print("--- subprocess stdout ---")\n    print(run.stdout[-1200:])\n    recs = [ln.strip() for ln in out.splitlines() if ln.lstrip().startswith("=========")]\n    print(f"--- {len(recs)} sanitizer lines from the upstream-only path ---")\n    for ln in recs[:45]:\n        print(ln[:190])\n'
        ),
        _markdown(
